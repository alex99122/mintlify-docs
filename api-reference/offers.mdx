...
<ParamField body="id" type="integer" required>
  Steam **assetId** of the listed item (numeric). The server loads the backpack row with `assetId` = this value, `state = 1`, and seller different from you.
</ParamField>

<ParamField body="price" type="integer" required>
  Offer price in cents (must be &gt;= 0; validated against listing and balance in `createOffer`)
</ParamField>
