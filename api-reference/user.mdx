---
title: "User"
description: "Account, balance, sessions, store profile, and history (Connected + API)"
---

Base URL: `https://api.mannco.store`. These routes require **Connected + API** unless noted: send `Authorization: Bearer <jwt>` from `POST /user/login`.

---

## Disconnect

```
GET /user/disconnect
```

**Permission:** Connected + API

Invalidates the current session / token (see server `disconnectSession()`).

### Response

```json
{
  "err": false,
  "success": true,
  "content": { "disconnect": true }
}
```

---

## Get user information

```
GET /user/infos
```

**Permission:** Connected + API

### Response (shape)

`content.informations` includes account fields such as `steamId`, `balance` (cents), `tradeurl`, `name`, `image`, `2fa` (string `"true"` / `"false"`), `shorturl`, `notification`, etc.

---

## Get balance

```
GET /user/balance
```

**Permission:** Connected + API

### Response

```json
{
  "err": false,
  "success": true,
  "content": { "balance": 250000 }
}
```

---

## Notifications

```
GET /user/notifications
```

**Permission:** Connected + API

Returns aggregated unread data (alerts, payment reviews, messages, offers, trades). Typical shape:

```json
{
  "err": false,
  "success": true,
  "content": {
    "alerts": [],
    "alertCount": 0,
    "paymentReviews": [],
    "paymentReviewsCount": 0,
    "messages": [],
    "messagesCount": 0,
    "offers": [],
    "offersCount": 0,
    "trades": [],
    "tradesCount": 0
  }
}
```

---

## IP / session list

```
GET /user/ipList
```

**Permission:** Connected + API

### Query parameters

<ParamField query="page" type="integer" default="0">
  Page index (multiplied by page size for offset).
</ParamField>

<ParamField query="perPage" type="integer" default="10">
  Page size (max **50**). Note camelCase **`perPage`**.
</ParamField>

<ParamField query="expire" type="string">
  Pass `"true"` to include expired sessions.
</ParamField>

### Response

`content.values` — session rows (ip, location, isp, `current`, timestamps, etc.) and `content.count` total.

---

## Public store profile

```
GET /user/store/{identifier}
```

**Permission:** API route filter only — **no Bearer required** for lookup.

<ParamField path="identifier" type="string" required>
  Store short URL / identifier.
</ParamField>

Returns minimal public info, e.g. `steamId`, `image`, `name`; or an error if not found.

---

## Sales stats

```
GET /user/getSalesInfos
```

**Permission:** Connected + API

Summary sales metrics for the authenticated user (`content` fields depend on `module/user` implementation).

---

## Sales chart

```
GET /user/getSalesChartInfos
```

**Permission:** Connected + API

### Query parameters

Optional `period`, and `chart` / `chart_only` for chart-specific responses (see handler).

---

## Balance history

```
GET /user/getBalanceHistory
```

**Permission:** Connected + API

<ParamField query="page" type="integer" default="0">
  Page number.
</ParamField>

<ParamField query="limit" type="integer" default="10">
  Page size (max **50**).
</ParamField>

First page (`page` = 0) returns `values` and `count`; later pages may return only `values`.

---

## Purchase history

```
GET /user/getPurchaseHistory
```

**Permission:** Connected + API

<ParamField query="page" type="integer" />
<ParamField query="count" type="integer">
  Page size (max **50**).
</ParamField>

---

## Sales history

```
GET /user/getSalesHistory
```

**Permission:** Connected + API

An admin variant `GET /user/getSalesHistory/{userid}` (**Connected + admin**) returns the sales history of another user.

### Query parameters

<ParamField query="page" type="integer" default="0">
  Page number (multiplied by `perPage` for the offset).
</ParamField>

<ParamField query="perPage" type="integer" default="10">
  Page size (max **50**). Note camelCase **`perPage`**.
</ParamField>

<ParamField query="range" type="string" default="1W">
  Time window: `1W` (7 days), `1M` (31 days), `1Y` (365 days), or `All` / `all` (no limit). Any other value falls back to `1W`.
</ParamField>

<ParamField query="search" type="string">
  Optional search term. Must be **at least 3 characters**, otherwise the request fails with `Search must be at least 3 characters`.
</ParamField>

### Response

First page (`page` = 0) returns `values` and `count`; later pages return only `values`.

---

## Cashout history

```
GET /user/getCashoutHistory
```

**Permission:** Connected + API

Supports `page` and one of `count`, `perpage`, or `limit` for page size (max **50**).

---

## Transaction history

```
GET /user/getTransactionHistory
```

**Permission:** Connected + API

<ParamField query="page" type="integer" />
<ParamField query="limit" type="integer">
  Max **50** (default paging applies).
</ParamField>

Optional `search`.

---

## Transaction details

```
GET /user/getTransactionDetails?transactionId={id}
```

**Permission:** Connected + API

<ParamField query="transactionId" type="string" required>
  Transaction identifier.
</ParamField>

### Response

`content.transaction` holds detail payload, or error `Transaction not found or access denied`.

---
